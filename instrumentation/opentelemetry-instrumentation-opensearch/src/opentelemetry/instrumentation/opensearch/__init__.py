# Copyright The OpenTelemetry Authors
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""
This library allows tracing HTTP opensearch made by the
`opensearch-py <https://opensearch.org/docs/latest/clients/python-api/>`_ library.

Usage
-----

.. code-block:: python

    from opentelemetry.instrumentation.opensearch import OpenSearchInstrumentor
    import opensearchpy
    from datetime import datetime

    # instrument opensearch
    OpenSearchInstrumentor().instrument()

    # Using opensearch as normal now will automatically generate spans
    client = opensearchpy.OpenSearch()
    client.index(index='my-index', id=1, body={'my': 'data', 'timestamp': datetime.now()})
    client.get(index='my-index', id=1)

OpenSearch instrumentation prefixes operation names with the string "OpenSearch". This
can be changed to a different string by either setting the OTEL_PYTHON_OPENSEARCH_NAME_PREFIX
environment variable or by passing the prefix as an argument to the instrumentor. For example,


.. code-block:: python

    from opentelemetry.instrumentation.opensearch import OpenSearchInstrumentor

    OpenSearchInstrumentor("my-custom-prefix").instrument()

The instrument() method accepts the following keyword args:
tracer_provider (TracerProvider) - an optional tracer provider
request_hook (Callable) - a function with extra user-defined logic to be performed before performing the request
this function signature is:
def request_hook(span: Span, method: str, url: str, kwargs)

response_hook (Callable) - a function with extra user-defined logic to be performed after performing the request
this function signature is:
def response_hook(span: Span, response: dict)

for example:

.. code-block: python

    from opentelemetry.instrumentation.opensearch import OpenSearchInstrumentor
    import opensearchpy
    from datetime import datetime

    def request_hook(span, method, url, kwargs):
        if span and span.is_recording():
            span.set_attribute("custom_user_attribute_from_request_hook", "some-value")

    def response_hook(span, response):
        if span and span.is_recording():
            span.set_attribute("custom_user_attribute_from_response_hook", "some-value")

    # instrument opensearch with request and response hooks
    OpenSearchInstrumentor().instrument(request_hook=request_hook, response_hook=response_hook)

    # Using opensearch as normal now will automatically generate spans,
    # including user custom attributes added from the hooks
    client = opensearchpy.OpenSearch()
    client.index(index='my-index', id=1, body={'my': 'data', 'timestamp': datetime.now()})
    client.get(index='my-index', id=1)

API
---
"""

import re
import warnings
from logging import getLogger
from os import environ
from typing import Collection

import opensearchpy
import opensearchpy.exceptions
import opensearchpy.transport
from wrapt import wrap_function_wrapper as _wrap

from opentelemetry.instrumentation.opensearch.package import _instruments
from opentelemetry.instrumentation.opensearch.version import __version__
from opentelemetry.instrumentation.instrumentor import BaseInstrumentor
from opentelemetry.instrumentation.utils import unwrap
from opentelemetry.semconv._incubating.attributes.db_attributes import (
    DB_STATEMENT,
    DB_SYSTEM,
)
from opentelemetry.trace import SpanKind, Status, StatusCode, get_tracer

from .utils import sanitize_body

logger = getLogger(__name__)


# Values to add as tags from the actual
# payload returned by OpenSearch, if any.
_ATTRIBUTES_FROM_RESULT = [
    "found",
    "timed_out",
    "took",
]

_DEFAULT_OP_NAME = "request"


class OpenSearchInstrumentor(BaseInstrumentor):
    """An instrumentor for opensearch
    See `BaseInstrumentor`
    """

    def __init__(self, span_name_prefix=None):
        if not span_name_prefix:
            span_name_prefix = environ.get(
                "OTEL_PYTHON_OPENSEARCH_NAME_PREFIX",
                "OpenSearch",
            )
        self._span_name_prefix = span_name_prefix.strip()
        super().__init__()

    def instrumentation_dependencies(self) -> Collection[str]:
        return _instruments

    def _instrument(self, **kwargs):
        """
        Instruments OpenSearch module
        """
        tracer_provider = kwargs.get("tracer_provider")
        tracer = get_tracer(
            __name__,
            __version__,
            tracer_provider,
            schema_url="https://opentelemetry.io/schemas/1.11.0",
        )
        request_hook = kwargs.get("request_hook")
        response_hook = kwargs.get("response_hook")
        _wrap(
            opensearchpy,
            "Transport.perform_request",
            _wrap_perform_request(
                tracer,
                self._span_name_prefix,
                request_hook,
                response_hook,
            ),
        )

    def _uninstrument(self, **kwargs):
        unwrap(opensearchpy.Transport, "perform_request")


_regex_doc_url = re.compile(r"/_doc/([^/]+)")

# search api https://opensearch.org/docs/latest/api-reference/search/
_regex_search_url = re.compile(r"/([^/]+)/_search[/]?")


# pylint: disable=too-many-statements
def _wrap_perform_request(
    tracer,
    span_name_prefix,
    request_hook=None,
    response_hook=None,
):
    # pylint: disable=R0912,R0914
    def wrapper(wrapped, _, args, kwargs):
        method = url = None
        try:
            method, url, *_ = args
        except IndexError:
            logger.warning(
                "expected perform_request to receive two positional arguments. "
                "Got %d",
                len(args),
            )

        op_name = span_name_prefix + (url or method or _DEFAULT_OP_NAME)

        doc_id = None
        search_target = None

        if url:
            # TODO: This regex-based solution avoids creating an unbounded number of span names, but should be replaced by instrumenting individual OpenSearch methods instead of Transport.perform_request()
            # A limitation of the regex is that only the '_doc' mapping type is supported. Mapping types are deprecated since OpenSearch 2.0
            match = _regex_doc_url.search(url)
            if match is not None:
                # Remove the full document ID from the URL
                doc_span = match.span()
                op_name = (
                    span_name_prefix
                    + url[: doc_span[0]]
                    + "/_doc/:id"
                    + url[doc_span[1] :]
                )
                # Put the document ID in attributes
                doc_id = match.group(1)
            match = _regex_search_url.search(url)
            if match is not None:
                op_name = span_name_prefix + "/<target>/_search"
                search_target = match.group(1)

        params = kwargs.get("params", {})
        body = kwargs.get("body", None)

        with tracer.start_as_current_span(
            op_name,
            kind=SpanKind.CLIENT,
        ) as span:
            if callable(request_hook):
                request_hook(span, method, url, kwargs)

            if span.is_recording():
                attributes = {
                    DB_SYSTEM: "opensearch",
                }
                if url:
                    attributes["opensearch.url"] = url
                if method:
                    attributes["opensearch.method"] = method
                if body:
                    # Don't set db.statement for bulk requests, as it can be very large
                    if isinstance(body, dict):
                        attributes[DB_STATEMENT] = sanitize_body(body)
                if params:
                    attributes["opensearch.params"] = str(params)
                if doc_id:
                    attributes["opensearch.id"] = doc_id
                if search_target:
                    attributes["opensearch.target"] = search_target
                for key, value in attributes.items():
                    span.set_attribute(key, value)

            try:
                rv = wrapped(*args, **kwargs)
            except opensearchpy.exceptions.OpenSearchException as ex:
                span.set_status(
                    Status(
                        status_code=StatusCode.ERROR,
                        description=str(ex),
                    )
                )
                raise

            body = rv
            if isinstance(body, dict) and span.is_recording():
                for member in _ATTRIBUTES_FROM_RESULT:
                    if member in body:
                        span.set_attribute(
                            f"opensearch.{member}",
                            str(body[member]),
                        )

            if callable(response_hook):
                response_hook(span, body)
            return rv

    return wrapper